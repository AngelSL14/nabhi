package us.gonet.statement.core.i8583.impl;
...
import us.gonet.statement.core.i8583.IISOBuilder;
...
public class ResponseHandler implements IResponseHandler {
    ...
    public ResponseWrapper< ATMResponseModel > sendMessage( ATMRequestModel wm ){
        ...
        try {
            ISO r0205 = isoBuilder.buildMessage( wm );
            ISO r0215 = sendISOMessage.sendMessage( r0205 );
            if ( !r0215.isTimeOut() ){
                handlerResponse( wm, r0215, responseWrapper );
                return responseWrapper;
            }else {
                List< ATMResponseModel > body = new ArrayList <>();
                List< ErrorWS > errors = new ArrayList <>();
                ErrorWS error = new ErrorWS();
                error.setCause( "Response received to late" );
                errors.add( error );
            ...
            LOG.error( "ServerException ResponseHandler STM" );
            List< ATMResponseModel > body = new ArrayList <>();
            ATMResponseModel r = new ATMResponseModel();
            r.setReceipt( "" );
    ...
    private void handlerResponse ( ATMRequestModel wm, ISO r0215, ResponseWrapper< ATMResponseModel > responseWrapper ) {
        List< ATMResponseModel > body = new ArrayList <>();
        ATMResponseModel r = new ATMResponseModel();
        responseWrapper.setCode( r0215.getDataElements().get( 38 ).getContentField() );
        body.add( r );
        responseWrapper.setBody( body );
        if ( r0215.getDataElements().get( 38 ).getContentField().equals( ResponseCodes.APPROVED.getValue() ) ){
            try {
                r.setReceipt( receiptIO.sendReceiptRequest( wm, r0215.getMessage() ).getTicket() );
            } catch ( ServerException e ) {
                LOG.error( "No se pudo crear el recibo mediante el servicio" );
            }
        }
        r.setMessage( r0215 );
    }
