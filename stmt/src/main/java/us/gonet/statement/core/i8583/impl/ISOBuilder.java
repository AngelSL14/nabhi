...
import us.gonet.iso8583.message.Request0205;
...
public class ISOBuilder implements IISOBuilder {
    ...
    public ISO buildMessage ( ATMRequestModel wm ) throws ServerException {
        ...
        dataElements.put( "p3", TranCodes.STATEMENT_PRINT.getValue() + wm.getFromAccount() + wm.getToAccount() );
        ...
        if ( wm.getEmv() != null ) {
            ...
            t.add( tB3.getId(), tB3 );
            t.add( tB4.getId(), tB4 );
            request0205 = new Request0205( dataElements, t );
        } else {
            request0205 = new Request0205( dataElements );
        }
        if ( LOG.isDebugEnabled() ) {
            LOG.debug( request0205.toString() );
        }
        return request0205.getIso();
    }
}
